-- ============================================================================
-- IMS - Inventory Management System + CRM
-- Скрипт инициализации базы данных PostgreSQL
-- Версия: 3.0 (CRM + Платформы + Заказ-наряды)
...
-- ============================================================================
-- 2. TABLES - Создаются в порядке зависимостей (без FK сначала)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS car_platforms (
    ...
    platform_confidence VARCHAR(20) DEFAULT 'unknown', -- verified, probable, unknown
    notes TEXT,
    created_at TIMESTAMP DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS car_legal_info (
    ...
    is_zalogoviy BOOLEAN DEFAULT FALSE, -- Исправлено: было "is залоговый"
    notes TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- 2.6 CRM - Клиенты (расширенные контрагенты)
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS crm_customers (
    ...
    address_actual TEXT,
    preferred_contact_method VARCHAR(20) DEFAULT 'phone', -- phone, sms, telegram, email
    preferred_service_time VARCHAR(50), -- утро, день, вечер, любой
    preferred_master INTEGER,
    source VARCHAR(50), -- сайт, рекомендации, реклама, проходной
...
CREATE TABLE IF NOT EXISTS services (
    ...
    price_type VARCHAR(20) DEFAULT 'fixed', -- fixed, hourly, percentage
...
CREATE TABLE IF NOT EXISTS work_orders (
    ...
    vehicle_info JSONB,
    status VARCHAR(30) DEFAULT 'draft', -- draft, accepted, in_progress, waiting_parts, ready, completed, cancelled, archived
...
CREATE TABLE IF NOT EXISTS work_order_items (
    ...
    item_type VARCHAR(20) NOT NULL, -- labor, part, material, service
    service_id INTEGER,
    part_id INTEGER,
    car_part_id INTEGER,
    name VARCHAR(255) NOT NULL,
    ...
    started_at TIMESTAMP,
    completed_at TIMESTAMP,
    part_source VARCHAR(20), -- warehouse, dismantling, external, customer
...
-- 2.15 CRM - История изменений заказ-наряда
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS work_order_history (
    ...
    attachment_type VARCHAR(20) -- photo, document, signature
);

-- 2.16 CRM - Вложения (фото, документы)
-- ----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS work_order_attachments (
    ...
    work_order_id INTEGER NOT NULL,
    item_id INTEGER,
    attachment_type VARCHAR(30) NOT NULL, -- photo_before, photo_after, document, signature, video
...
CREATE TABLE IF NOT EXISTS payments (
    ...
    payment_method VARCHAR(30) NOT NULL, -- cash, card, transfer, online, mixed
...
CREATE TABLE IF NOT EXISTS customer_notifications (
    ...
    channel VARCHAR(20) NOT NULL, -- sms, email, telegram, whatsapp, push
...
-- ============================================================================
-- 3. FOREIGN KEYS (Внешние ключи - добавляем после создания всех таблиц)
-- ============================================================================
...
ALTER TABLE cars 
    ADD CONSTRAINT fk_cars_created_by 
    FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL;

ALTER TABLE car_platforms 
    ADD CONSTRAINT fk_car_platforms_car 
    FOREIGN KEY (car_id) REFERENCES cars(id) ON DELETE CASCADE;

ALTER TABLE car_platforms 
    ADD CONSTRAINT fk_car_platforms_code 
    FOREIGN KEY (platform_code) REFERENCES vehicle_platforms(platform_code);

ALTER TABLE car_compatibility 
    ADD CONSTRAINT fk_compatibility_source_car 
    FOREIGN KEY (source_car_id) REFERENCES cars(id) ON DELETE CASCADE;

ALTER TABLE car_legal_info 
    ADD CONSTRAINT fk_legal_info_car 
    FOREIGN KEY (car_id) REFERENCES cars(id) ON DELETE CASCADE;

ALTER TABLE car_parts 
    ADD CONSTRAINT fk_car_parts_car 
    FOREIGN KEY (car_id) REFERENCES cars(id) ON DELETE CASCADE;

ALTER TABLE car_parts 
    ADD CONSTRAINT fk_car_parts_item 
...
ALTER TABLE crm_customers 
    ADD CONSTRAINT fk_crm_customer_counterparty 
    FOREIGN KEY (counterparty_id) REFERENCES counterparties(id) ON DELETE SET NULL;

ALTER TABLE crm_customers 
    ADD CONSTRAINT fk_crm_customer_preferred_master 
    FOREIGN KEY (preferred_master) REFERENCES users(id) ON DELETE SET NULL;
...
ALTER TABLE work_orders 
    ADD CONSTRAINT fk_wo_accepted_by 
    FOREIGN KEY (accepted_by) REFERENCES users(id) ON DELETE SET NULL;

ALTER TABLE work_orders 
    ADD CONSTRAINT fk_wo_assigned_master 
    FOREIGN KEY (assigned_master) REFERENCES users(id) ON DELETE SET NULL;

ALTER TABLE work_orders 
    ADD CONSTRAINT fk_wo_assigned_bay 
    FOREIGN KEY (assigned_bay) REFERENCES locations(id) ON DELETE SET NULL;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_work_order 
    FOREIGN KEY (work_order_id) REFERENCES work_orders(id) ON DELETE CASCADE;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_service 
    FOREIGN KEY (service_id) REFERENCES services(id) ON DELETE SET NULL;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_part 
    FOREIGN KEY (part_id) REFERENCES items(id) ON DELETE SET NULL;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_car_part 
    FOREIGN KEY (car_part_id) REFERENCES car_parts(id) ON DELETE SET NULL;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_performed_by 
    FOREIGN KEY (performed_by) REFERENCES users(id) ON DELETE SET NULL;

ALTER TABLE work_order_items 
    ADD CONSTRAINT fk_woi_supplier 
    FOREIGN KEY (supplier_id) REFERENCES suppliers(id) ON DELETE SET NULL;

ALTER TABLE work_order_history 
    ADD CONSTRAINT fk_woh_work_order 
    FOREIGN KEY (work_order_id) REFERENCES work_orders(id) ON DELETE CASCADE;

ALTER TABLE work_order_history 
    ADD CONSTRAINT fk_woh_changed_by 
    FOREIGN KEY (changed_by) REFERENCES users(id) ON DELETE SET NULL;

ALTER TABLE work_order_attachments 
    ADD CONSTRAINT fk_woa_work_order 
    FOREIGN KEY (work_order_id) REFERENCES work_orders(id) ON DELETE CASCADE;

ALTER TABLE work_order_attachments 
    ADD CONSTRAINT fk_woa_item 
    FOREIGN KEY (item_id) REFERENCES work_order_items(id) ON DELETE CASCADE;

ALTER TABLE work_order_attachments 
    ADD CONSTRAINT fk_woa_taken_by 
...
ALTER TABLE customer_notifications 
    ADD CONSTRAINT fk_notifications_work_order 
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_woi_type') THEN
        ALTER TABLE work_order_items ADD CONSTRAINT chk_woi_type 
            CHECK (item_type IN ('labor', 'part', 'material', 'service'));
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_loyalty_level') THEN
        ALTER TABLE crm_customers ADD CONSTRAINT chk_loyalty_level 
...
CREATE TRIGGER trg_car_platforms_updated_at 
    BEFORE UPDATE ON car_platforms 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
INSERT INTO services (code, name, category, base_price, labor_hours_standard, is_active) VALUES
('DIAGNOSTICS', 'Диагностика автомобиля', 'Диагностика', 1500.00, 1.0, TRUE),
('TO-15000', 'ТО-15000 км', 'Техническое обслуживание', 5000.00, 2.0, TRUE),
('OIL-CHANGE', 'Замена масла ДВС', 'Техническое обслуживание', 800.00, 0.5, TRUE),
('BRAKE-FRONT', 'Замена тормозных колодок (перед)', 'Тормозная система', 1200.00, 1.0, TRUE),
('SUSPENSION-REV', 'Ревизия подвески', 'Ходовая часть', 2000.00, 2.0, TRUE)
...
COMMENT ON TABLE crm_customers IS 'Клиенты автосервиса (расширенные контрагенты)';
...
COMMENT ON COLUMN crm_customers.loyalty_level IS 'bronze=Базовый, silver=Серебро, gold=Золото, platinum=Платина';
...
SELECT '✅ Database initialization completed successfully!' AS status,
       ...
       (SELECT COUNT(*) FROM vehicle_platforms) as platforms_count;